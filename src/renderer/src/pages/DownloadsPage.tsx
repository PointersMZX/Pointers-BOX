    ...
    >
      <HStack justify="space-between" mb={2}>
        ...
        <HStack flexShrink={0} spacing={1}>
          <Text fontSize="xs" color="ptextmuted">
            ...
            {task.bytesPerSecond > 0 && !task.paused
              ? ` · ${formatBytes(task.bytesPerSecond)}/s`
              : ''}
          </Text>
          {/* 更新包由 updater 管理，不提供暂停/取消 */}
          {task.source === 'browser' && (
            <>
              <Tooltip label={task.paused ? '继续下载' : '暂停下载'}>
                <IconButton
                  aria-label={task.paused ? '继续下载' : '暂停下载'}
                  icon={task.paused ? <FiPlay /> : <FiPause />}
                  size="xs"
                  variant="ghost"
                  color="ptext"
                  onClick={() => void (task.paused ? resume(task.id) : pause(task.id))}
                />
              </Tooltip>
              <Tooltip label="取消下载">
                <IconButton
      ...
      <HStack spacing={3}>
        <Progress
          flex="1"
          size="sm"
          colorScheme={task.paused ? 'gray' : 'brand'}
          value={percent}
          hasStripe={!task.paused}
          isAnimated={!task.paused}
        />
        <Text fontSize="xs" color="ptextmuted" w="64px" textAlign="right">
          {task.paused ? '已暂停' : `${percent.toFixed(0)}%`}
        </Text>
      </HStack>
    </Box>
  )
