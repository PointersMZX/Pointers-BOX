...
const THEME_KEYS = ["glass", "black", "white"];
const DEFAULT_THEME = "glass";
const DEFAULT_ACCENT = "#3182ce";
function normalizeTheme(v) {
  if (typeof v === "string" && THEME_KEYS.includes(v)) {
    return v;
  }
...
const attachedSessions = /* @__PURE__ */ new WeakSet();
let counter = 0;
...
function attachDownloadHandling(target = getBrowserSession()) {
  ...
  target.on("will-download", (_event, item) => {
    ...
    const savePath = pickAvailablePath(fs.existsSync, dir, filename);
    ...
    item.once("done", (_e, state2) => {
      ...
      if (state2 === "cancelled" && fs.existsSync(rec.path)) {
        try {
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("downloads:cancel", (_e, id) => cancelDownload(String(id ?? "")));
  electron.ipcMain.handle("downloads:pause", (_e, id) => pauseDownload(String(id ?? "")));
  electron.ipcMain.handle("downloads:resume", (_e, id) => resumeDownload(String(id ?? "")));
...
function injectNavOverride(contents) {
